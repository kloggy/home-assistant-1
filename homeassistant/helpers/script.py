...
class _ScriptRun:
    ...
    async def async_run(self) -> None:
        ...
        for cur, action in islice(enumerate(self._script.sequence), self._cur, None):
            try:
                ...
                if self._script._change_listener:
                    self.hass.async_add_job(self._script._change_listener)
                return
            except _StopScript:
                break
    ...
    def _async_log_exception(self, step, action, exception):
        ...
        error = None
        meth = self._script._logger.error  # pylint: disable=protected-access
        ...
        if isinstance(exception, vol.Invalid):
            ...
            # pylint: disable=protected-access
            meth = self._script._logger.exception
            error = ""
    ...
    async def _async_delay(self, action):
        ...
        try:
            ...
            self._log("Error rendering delay template: %s", ex, level=logging.ERROR)
            raise _StopScript
    ...
    async def _async_fire_event(self, action):
        ...
        self.hass.bus.async_fire(action[CONF_EVENT], event_data, context=self._context)

    async def _async_check_condition(self, action):
        """Test if condition is matching."""
        config_cache_key = frozenset((k, str(v)) for k, v in action.items())
        # pylint: disable=protected-access
        config = self._script._config_cache.get(config_cache_key)
        if not config:
            config = await condition.async_from_config(self.hass, action, False)
            self._script._config_cache[config_cache_key] = config
    ...
    def _log(self, msg, *args, level=logging.INFO):
        # pylint: disable=protected-access
        self._script._logger.log(level, msg, *args)
...
class Script:
    ...
    def __init__(
        ...
        if mode:
            ...
            self._mode = SCRIPT_PARALLEL_RESTART
        else:
            self._mode = SCRIPT_PARALLEL_ALLOW
    ...
    async def async_run(
        ...
        if self.is_running:
            ...
            if self._mode == SCRIPT_PARALLEL_ERROR:
                self._log("Already running", level=logging.ERROR)
        ...
        run = _ScriptRun(self.hass, self, variables, context)
